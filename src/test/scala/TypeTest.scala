...
class TypeTest extends FlatSpec with Matchers {
  ...
  "lambda expression type" should "FunctionT" in {
    val lam = new LambdaExpression(("x", IntT), IntT, new SumExpression(IntValue(1), IntValue(1)))
  ...
  "lambda application type" should "IntT" in {
    ...
    val lam = new LambdaExpression(("x", IntT), IntT, new SumExpression(refX, IntValue(1)))
